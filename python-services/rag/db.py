...
def _row_to_pair(row) -> dict:
    ...
    topic = row["subject_slug"] or _normalize_topic(row["post_title"])
    return {
        "id": f"db-post-{row['id']}",
...
# LEFT JOIN Subject — uses slug as topic (reliable); falls back to post title if no subject.
_QUERY = """
    SELECT
        p.id,
    ...
    WHERE p.deleted_at IS NULL
      ...
      AND p.content     <> ''
    GROUP BY p.id, p.title, s.slug, p.content
"""
