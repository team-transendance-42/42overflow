"""
Populate Postgres with test Users + Posts + Comments for manual RAG testing.
...
Usage (stack must be up, run from python-services/rag/):
  uv run python dev_populate.py           # insert in rag chromadb all fake posts
  uv run python dev_populate.py --clean   # delete bot posts first, re-insert

After running, trigger RAG sync (from project root):  # forbidden-todo
docker compose exec python-rag curl -s -X POST http://localhost:8090/admin/reload-from-db -H "X-Admin-Token: ${RAG_ADMIN_TOKEN}"
...
SUBJECTS = [
    {"name": "push_swap",    "slug": "push_swap",    "description": "Stack sorting algorithm project"},
    {"name": "minishell",    "slug": "minishell",    "description": "Mini Unix shell implementation"},
    {"name": "webserv",      "slug": "webserv",      "description": "HTTP/1.1 web server in C++"},
    {"name": "philosophers", "slug": "philosophers", "description": "Dining philosophers concurrency problem"},
    {"name": "ft_printf",    "slug": "ft_printf",    "description": "Reimplementation of printf"},
]
...
async def ensure_subjects(conn) -> dict[str, int]:
    for s in SUBJECTS:
        await conn.execute(
            """
            INSERT INTO "Subject" (name, slug, description, created_at, updated_at)
            VALUES ($1, $2, $3, NOW(), NOW())
            ON CONFLICT (slug) DO NOTHING
            """,
            s["name"], s["slug"], s["description"],
        )
    rows = await conn.fetch('SELECT id, slug FROM "Subject"')
    mapping = {row["slug"]: row["id"] for row in rows}
    print(f"[subjects] ready: {', '.join(s['slug'] for s in SUBJECTS)}")
    return mapping
...
async def insert_posts(conn) -> tuple[int, int]:
    ...
    subjects = await ensure_subjects(conn)

    for entry in POSTS:
        slug = entry["title"]
        subject_id = subjects.get(slug)
        if subject_id is None:
            print(f"[warn] no Subject with slug '{slug}' — skipping post: {entry['content'][:55]}...")
            skipped += 1
            continue
